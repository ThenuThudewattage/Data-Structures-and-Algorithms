...
struct HashTable{
    int MAX_LENGTH = 4;
    string * password = new string[MAX_LENGTH]; //pointer to password -- a string of 4 characters

    void intialize_hashtable(){
        for (int i = 0; i < 4; i++){
            password[i].clear(); //clear the string
        }
    }

    bool isFull(){ //check if the string is full
        bool full = true;
        int count = 0;
    ...
    int hashfunc(string user_name){  //implementation of hash function
        int sum = 0;
        int hash = 0;
	//add your code below
    ...
    bool is_slot_empty(int hash){
        bool empty = password[hash].empty();  //It returns true if the set container is empty (size is 0) otherwise, it returns false.
        return empty;
    }
    ...
    void insert(string user_name,string user_password){
        ...
        if (!empty){
            cout<<"Cannot insert, Collision caused." << endl;
            return;
        } else{
    ...
    void hash_lookup(string user_name){
	    ...
	    //add an if condition to complete the code here
        if(empty){
            cout<< user_name << "does not exists."<<"\n";
            return;
        }
        cout<<"Password for "<< user_name << " is " << password[hash] << "\n";	
    }

    void delete_item(string user_name){ //delete item 
        int hash;
        bool empty;
        hash = hashfunc(user_name);
        empty = is_slot_empty(hash);
